...
void main() {
  test('SmartTurnDetector runs real model and outputs sane probability', () async {
    // 1. Locate the model. In tests, the CWD is typically the package root.
    final modelFile = File('../pipecat_smart_turn_platform_interface/assets/smart-turn-v3.2-cpu.onnx');
    ...
    // This initialization is expected to throw SmartTurnModelLoadException if FFI fails to load onnxruntime,
    // which happens in pure Dart tests if we don't manually load the DLL/so.
    // We will skip if onnxruntime is missing rather than failing, as Dart test runners often lack native libs.
    try {
      await detector.initialize();
    } catch (e) {
      if (e.toString().contains('Failed to load ONNX model')) {
        markTestSkipped('Skipping real model test because ONNX Runtime native bindings are not available in this test environment. Error: $e');
        return;
      }
      rethrow;
    }
